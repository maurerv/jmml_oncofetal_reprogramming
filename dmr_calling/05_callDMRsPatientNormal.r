...
analysis.dir  <- "/omics/groups/OE0219/internal/jmmlc_pbat_normals/data/odcf_md/analysis/220512_DMR_hierachy_HSC_PatientNormal"
...
normalStage   <- as.character(pheno$Developmental_Stage[normals])
pheno$group   <- as.character(pheno$Patient)
pheno$group[normals]   <- normalStage
groups        <- unique(pheno$group)
patients  <- as.character(pheno$Patient[!normals])
contrasts <- as.data.frame(
    t(expand.grid(unique(patients), unique(normalStage)))
)
...
for(i in names(cov_drms)){
    ...
    keepLoci[[i]] <- which(rowSums(cov_drms[[i]][, gr1, drop=F] >= 5) >= max(c(1,round(length( gr1)/2))) &
                     rowSums(cov_drms[[i]][, gr2, drop=F] >= 5) >= max(c(1, round(length( gr2)/2))))
}
