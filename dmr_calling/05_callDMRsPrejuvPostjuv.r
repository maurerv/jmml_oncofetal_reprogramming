...
analysis.dir  <- "/omics/groups/OE0219/internal/jmmlc_pbat_normals/data/odcf_md/analysis/220512_DMR_hierachy_HSC_PrejuvPostjuv"
...
normals   <- (pheno$Sample_Type == "normal") & (pheno$Celltype == "HSC")
normals   <- normals & !(pheno$Name %in% c("FL2_HSC_PBAT_2", "JU3_HSC_PBAT_11"))
normals   <- normals & !(pheno$Developmental_Stage %in% c("fetal"))
#complete  <- which(complete.cases(bsseq::getMeth(bsseq_all, type = "raw")))
#complete  <- complete[1:min(10000, length(complete))]
bsseq_all <- bsseq_all[, normals]
print(bsseq_all)

#define groups for dmr calling
pheno         <- pData(bsseq_all)
pheno$group   <- paste0(pheno$Developmental_Stage)
pheno_normal  <- pheno[grep("normal", pheno$Sample_Type, invert=FALSE),]
groups        <- pheno_normal$group
groups[groups %in% c("juvenile", "perinatal")] = "Prejuv"
groups[groups == "young_adult"] = "Postjuv"
pheno_normal$group = groups
...
dml_test = mclapply(as.data.frame(contrasts), function(contrast){
    ...
    gr1 <- rownames(pheno_normal[pheno_normal$group==c1,])
    gr2 <- rownames(pheno_normal[pheno_normal$group==c2,])
    DMLtest(bsseq_all, group1 = gr1, group2 = gr2, smoothing = TRUE,
        BPPARAM = mParam)
}, mc.cores = 4)
