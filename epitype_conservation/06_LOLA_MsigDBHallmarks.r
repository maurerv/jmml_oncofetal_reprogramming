...
saveRDS(results_regionDB, file.path(analysis.dir, "LOLA_resultTables", "results_MSigDB_hg19.rds"))
results_regionDB<- readRDS(file.path(analysis.dir, "LOLA_resultTables", "results_MSigDB_hg19.rds"))
...
for(i in names(results_regionDB)){
    ...
    for(j in unique(unique(result$collection))){
        ...
        pdf(file.path(analysis.dir,"LOLA", paste0("EnrichLOLA_", j, ".pdf")), width=15, height=15)
        print(g)
        dev.off()
    }
}
